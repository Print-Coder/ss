...
extension CapExtension on String {
  String get inCaps => this.length>0?'${this[0].toUpperCase()}${this.substring(1)}':this;
...
//   }
  ...
  static String getPrice(double myPrice) {
    ...
    var noDecimalformat = NumberFormat.currency(
        locale: 'HI', symbol: curencySymbol + " ", decimalDigits: 0);

    try {
      if (myPrice <= 0) {
        return noDecimalformat.format(0);
      }
      return format.format(myPrice.abs() ?? 0);
    } catch (e) {
      return '0';
    }
  }

  static String getDropDown(double myPrice) {
    var format = NumberFormat.currency(locale: 'HI', symbol: curencySymbol);
    var noDecimalformat = NumberFormat.currency(
        locale: 'HI', symbol: curencySymbol + " ", decimalDigits: 0);
    ...
    try {
      if (myPrice <= 0) {
        return noDecimalformat.format(0) + " ea";
      }
      return format.format(myPrice.abs() ?? 0) + " ea";
    } catch (e) {
      return '0';
    }
  }

  static String getOrderPrice(double myPrice) {
    var format =
        NumberFormat.currency(locale: 'HI', symbol: curencySymbol + " ");
    var noDecimalformat = NumberFormat.currency(
        locale: 'HI', symbol: curencySymbol + " ", decimalDigits: 0);
  ...
  //         maxLines: 1,
  //         text:
  //             // setting.value?.currencyRight != null &&
